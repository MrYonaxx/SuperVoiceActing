...
namespace DialoguerEditor{
    ...
    public class DialogueEditorMasterObject{
        ...
        private void sortDialogues()
        {
            ...
            {
                dialogues[i].listIndex = i;
            }
            dialogues.Sort( ( d1, d2 ) => d1.listIndex.CompareTo( d2.listIndex ) );
        }
        ...
        public void removeCurrentDialogue(){
            ...
            dialogues.RemoveAt( currentDialogueId );
            currentDialogueId = currentDialogueId;
            sortDialogues();
        }
        
        public void setDialogueIndex( int oldIndex, int newIndex ){
            if ( oldIndex < 0 || newIndex < 0 || oldIndex >= count || newIndex >= count )
                return;

            var item = dialogues[oldIndex];
            dialogues.RemoveAt( oldIndex );
            //if ( newIndex > oldIndex ) newIndex--;
            dialogues.Insert( newIndex, item );

            if ( currentDialogueId == oldIndex )
                currentDialogueId = newIndex;
            sortDialogues();
        }
        ...
        public DialoguerData getDialoguerData(){
            ...
            for(int d = 0; d<this.dialogues.Count; d+=1){
                ...
                DialoguerDialogue newDialogue = new DialoguerDialogue( dialogue.id, dialogue.name, dialogue.startPage, localVariables, newPhases);
