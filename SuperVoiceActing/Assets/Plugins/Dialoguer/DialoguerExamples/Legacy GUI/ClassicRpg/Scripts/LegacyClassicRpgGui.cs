...
public class LegacyClassicRpgGui : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (!_isBranchedText)
		{
			if (Input.GetMouseButtonDown(0) || Input.GetKeyDown(KeyCode.X) || Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.Return))
			{
				...
				{
					DialoguerDialogueManager.Continue(0);
				}
				else
				{
		...
		else
		{
			...
			if (Input.GetKeyDown(KeyCode.X) || Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.Return))
			{
				if (_windowCurrentText == _windowTargetText)
				{
                    DialoguerDialogueManager.Continue( _currentChoice);
				}
				else
				{
					_windowCurrentText = _windowTargetText;
	...
	{
		DialoguerEventManager.onStarted += onDialogueStartedHandler;
		DialoguerEventManager.onEnded += onDialogueEndedHandler;
		DialoguerEventManager.onSuddenlyEnded += onDialogueInstantlyEndedHandler;
		DialoguerEventManager.onTextPhase += onDialogueTextPhaseHandler;
		DialoguerEventManager.onWindowClose += onDialogueWindowCloseHandler;
		DialoguerEventManager.onMessageEvent += onDialoguerMessageEvent;
	}

	private void onDialogueStartedHandler( int dialogueID )
	{
		_dialogue = true;
	}

	private void onDialogueEndedHandler( int dialogueID )
	{
		_ending = true;
		audioTextEnd.Play();
	}

	private void onDialogueInstantlyEndedHandler( int dialogueID )
	{
		_dialogue = false;
		_showDialogueBox = false;
		resetWindowSize();
