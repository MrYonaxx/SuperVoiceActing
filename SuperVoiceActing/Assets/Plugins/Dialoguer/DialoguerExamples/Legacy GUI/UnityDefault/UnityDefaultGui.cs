...
public class UnityDefaultGui : MonoBehaviour {
	...
	void OnGUI(){
		...
		for(int i = 0; i<_choices.Length; i+=1){
			...
			if(GUI.Button(buttonRect, _choices[i])){
                ...
                DialoguerDialogueManager.Continue( i );
			}
		}
	...
	public void addDialoguerEvents(){
		DialoguerEventManager.onStarted += onStartedHandler;
		DialoguerEventManager.onEnded += onEndedHandler;
		DialoguerEventManager.onSuddenlyEnded += onInstantlyEndedHandler;
		DialoguerEventManager.onTextPhase += onTextPhaseHandler;
		DialoguerEventManager.onWindowClose += onWindowCloseHandler;
	}
	
	private void onStartedHandler( int dialogueID )
    {
		_showing = true;
	}
	
	private void onEndedHandler( int dialogueID )
    {
		_showing = false;
	}
	
	private void onInstantlyEndedHandler( int dialogueID )
    {
		_showing = true;
