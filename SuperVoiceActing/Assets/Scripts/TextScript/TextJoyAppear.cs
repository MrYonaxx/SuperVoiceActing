        ...
        protected override IEnumerator AnimateVertexColors()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < characterCount; i++)
                {
                    ...
                    if (vertAnim.offset > letterOffset * 2)
                    {
                        position = vertAnim.vertexPosition + new Vector3(0, vertAnim.offset - (letterOffset * 2.5f), 0);
                        vertAnim.offset -= gravity;
                        vertAnim.vertexPosition = position;
                    }
                    else if (vertAnim.offset >= letterOffset)
                    {
                        position = vertAnim.vertexPosition - new Vector3(0, (vertAnim.offset - (letterOffset * 1.5f)) * 0.5f, 0);
                        vertAnim.offset -= gravity * 2;
                        vertAnim.vertexPosition = position;
                    }
                    else if (vertAnim.offset >= 0)
                    {
                        position = vertAnim.vertexPosition + new Vector3(0, (vertAnim.offset - (letterOffset * 0.5f)) * 0.25f, 0);
                        vertAnim.offset -= gravity * 4;
                        vertAnim.vertexPosition = position;
                    }
                    else if (vertAnim.offset >= -letterOffset * 2)
                    {
                        vertAnim.offset -= gravity * 0.25f;
                    }
                    else
                    {
                        vertAnim.offset = letterOffset * 3;
                    }
