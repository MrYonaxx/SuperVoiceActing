...
namespace VoiceActing
{
    ...
    /// </summary>
    public class TextSurpriseAppear : TextPerformanceAppear
    {
        #region Attributes 
        ...
        /*protected override IEnumerator AnimateVertexColors()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < characterCount; i++)
                {
                    ...
                    Vector2 charMidBasline = new Vector2((sourceVertices[vertexIndex + 0].x + sourceVertices[vertexIndex + 2].x) / 2, charInfo.topRight.y);
                    ...
                    if(vertAnim.angle < -shake)
                    {
                        vertAnim.angle = Mathf.SmoothStep(-270, shake, Mathf.PingPong(loopCount / 25f * 0.2f, 1f));
                    }
                    else
                    {
                        vertAnim.angle = Mathf.SmoothStep(-shake, shake, Mathf.PingPong(loopCount / 25f * 0.5f, 1f));
                    }
                    ...
                    if (vertexAnim[i].selected == true)
                    {
                        ...
                        matrix = Matrix4x4.TRS(position, Quaternion.Euler(0, 0, vertAnim.angle), Vector3.one + new Vector3(size, size, size));
                    }
                    else
                    {
                        matrix = Matrix4x4.TRS(position, Quaternion.Euler(0, 0, vertAnim.angle), Vector3.one);
                    }
